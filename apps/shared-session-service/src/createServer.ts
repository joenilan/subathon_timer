...
const MAX_PARTICIPANTS_DEFAULT = 6
const DEFAULT_TIMER_SECONDS_DEFAULT = 21600
...
function createDefaultTimerState(defaultSeconds: number): SharedTimerState {
  return {
    timerStatus: 'paused',
    timerSessionBaseRemainingSeconds: clampTimerSeconds(defaultSeconds),
...
function resolveTimerRuntime(timerState: SharedTimerState, now = Date.now()) {
  if (timerState.timerStatus !== 'running' || !timerState.timerSessionRunningSince) {
    return {
      timerStatus:
        timerState.timerSessionBaseRemainingSeconds <= 0 && timerState.timerStatus !== 'idle'
          ? 'finished'
  ...
  const timerRemainingSeconds = clampTimerSeconds(
    timerState.timerSessionBaseRemainingSeconds - elapsedSeconds,
  )
  const uptimeSeconds = clampTimerSeconds(
    timerState.timerSessionBaseUptimeSeconds + elapsedSeconds,
  )
...
function applyTimerAction(
  ...
  const collapsed = {
    timerStatus: (
      runtime.timerRemainingSeconds <= 0
        ? 'finished'
        : wasRunning
          ? 'paused'
          : runtime.timerStatus
    ) as SharedTimerStatus,
  ...
  switch (action.action) {
    case 'start':
      if (collapsed.timerSessionBaseRemainingSeconds <= 0) {
        return { ...collapsed, timerStatus: 'finished' }
      }
    ...
    case 'reset':
      return createDefaultTimerState(defaultSeconds)

    case 'adjust': {
      const nextRemaining = clampTimerSeconds(
        collapsed.timerSessionBaseRemainingSeconds + action.deltaSeconds,
      )
    ...
    case 'set': {
      ...
      return {
        timerStatus:
          nextRemaining <= 0
            ? 'finished'
            : collapsed.timerStatus === 'idle'
              ? 'paused'
              : collapsed.timerStatus,
        timerSessionBaseRemainingSeconds: nextRemaining,
        timerSessionBaseUptimeSeconds: collapsed.timerSessionBaseUptimeSeconds,
        timerSessionRunningSince: null,
      }
...
export interface SharedSessionServer {
  port: number
  stop: () => Promise<void>
}

export function createSharedSessionServer(options: CreateServerOptions): SharedSessionServer {
  const { host = '127.0.0.1' } = options
  const maxParticipants = options.maxParticipants ?? MAX_PARTICIPANTS_DEFAULT
  const defaultTimerSeconds = options.defaultTimerSeconds ?? DEFAULT_TIMER_SECONDS_DEFAULT
  ...
  function applySharedTwitchEvent(
    ...
    const nextStatus: SharedTimerStatus =
      nextRemaining <= 0 ? 'finished' : runtime.timerStatus === 'running' ? 'running' : 'paused'
    const now = Date.now()
  ...
  function applySharedTipEvent(
    session: SessionRecord,
    participant: ParticipantRecord,
    event: NormalizedTwitchEvent,
  ) {
    if (
      (event.source !== 'streamelements' && event.source !== 'streamlabs') ||
      event.eventType !== 'tip'
    ) {
      return false
    }
  ...
  function addSocketToSession(sessionId: string, socket: ServerWebSocket<SharedSessionSocketData>) {
    const sockets =
      sessionSockets.get(sessionId) ?? new Set<ServerWebSocket<SharedSessionSocketData>>()
    sockets.add(socket)
  ...
  function removeSocketFromSession(
    sessionId: string,
    socket: ServerWebSocket<SharedSessionSocketData>,
  ) {
  ...
  function updateSessionStatus(session: SessionRecord) {
    ...
    if (session.status !== 'ended') {
      session.status =
        session.participants.length > 1 || connectedCount > 1
          ? 'active'
          : 'waiting_for_collaborators'
    }
  ...
  function endSession(session: SessionRecord) {
    ...
    // Remove from the registry FIRST so that close events triggered by
    // the sockets below don't re-broadcast a snapshot to remaining connections.
    ...
    for (const socket of sockets) {
      try {
        // Send the ended notification and let each client close their own
        // connection on receipt. Calling socket.close() here can block on
        // Bun's WebSocket close-handshake and is not necessary — the session
        // record already has status 'ended' so reconnect attempts will fail.
        socket.send(endedPayload)
      } catch {
  ...
  function consumeNextWheelTrigger(session: SessionRecord) {
    ...
    const eligibleSegments = getEligibleWheelSegmentsForGiftCount(
      session.wheelSegments,
      nextTrigger.giftCount,
    )
        ...
        if (selectedSegment.outcomeType !== 'timeout') {
          wheelAutoApplyTimers.set(
            ...
            setTimeout(() => {
              ...
              if (
                !sessionForApply ||
                sessionForApply.wheelSpin.activeSegmentId !== selectedSegment.id
              ) {
                return
              }
              ...
              const deltaSeconds =
                selectedSegment.outcomeType === 'time' ? (selectedSegment.timeDeltaSeconds ?? 0) : 0
              const nextRemaining = clampTimerSeconds(runtime.timerRemainingSeconds + deltaSeconds)
              const nextStatus: SharedTimerStatus =
                nextRemaining <= 0
                  ? 'finished'
                  : runtime.timerStatus === 'running'
                    ? 'running'
                    : 'paused'
              const now = Date.now()
              ...
              sessionForApply.recentActivity = prependSharedActivityEntry(
                sessionForApply.recentActivity,
                {
                  id: `wheel:${selectedSegment.id}:${now}`,
                  sourceParticipantId:
                    sessionForApply.wheelSpin.sourceParticipantId ??
                    sessionForApply.hostParticipantId,
                  sourceParticipantLabel:
                    sessionForApply.participants.find(
                      (participant) =>
                        participant.id === sessionForApply.wheelSpin.sourceParticipantId,
                    )?.displayName ?? 'Shared wheel',
                  provider: 'twitch',
                  eventType: 'gift_bomb',
                  title:
                    selectedSegment.outcomeType === 'time'
                      ? 'Wheel outcome applied'
                      : 'Wheel outcome logged',
                  summary:
                    selectedSegment.outcomeType === 'time'
  ...
  function createJoinToken(sessionId: string, participantId: string) {
    ...
    joinTokens.set(token, { token, sessionId, participantId })
    return token
  }
  ...
  function defaultRuntimeState(): ParticipantRuntimeState {
    return {
  ...
  const server = Bun.serve<SharedSessionSocketData>({
    ...
    fetch(request, server) {
      ...
      if (url.pathname === '/sessions' && request.method === 'POST') {
        return parseJsonBody(request).then((body) => {
          ...
          const session: SessionRecord = {
            ...
            timerState: createDefaultTimerState(defaultTimerSeconds),
            ruleConfig: normalizeTimerRuleConfig(
              payload?.ruleConfig ?? getDefaultTimerRuleConfig(),
            ),
            recentActivity: [],
            appliedEventKeys: [],
            wheelSegments:
              Array.isArray(payload?.wheelSegments) && payload.wheelSegments.length > 0
                ? payload.wheelSegments
                : [],
        ...
        return parseJsonBody(request).then((body) => {
          ...
          if (!session) {
            return errorResponse(
              'That invite code does not match an active shared session.',
              404,
            )
          }

          if (session.status === 'ended') {
            return errorResponse('That shared session has ended.', 410)
          }

          if (session.participants.length >= maxParticipants) {
            return errorResponse(
              `This shared session already has all ${maxParticipants} creator slots filled.`,
              409,
            )
          }
      ...
      // Rejoin: existing participant gets a fresh join token (handles app-restart scenario)
      const rejoinMatch = /^\/sessions\/([^/]+)\/rejoin$/.exec(url.pathname)
      if (rejoinMatch && request.method === 'POST') {
        return parseJsonBody(request).then((body) => {
          const sessionId = rejoinMatch[1]
          const payload = body as { participantId?: string } | null
          const participantId = payload?.participantId?.trim()

          if (!participantId) {
            return errorResponse('A participant ID is required to rejoin.')
          }

          const session = sessions.get(sessionId)
          if (!session) {
            return errorResponse('That session no longer exists.', 404)
          }
          ...
          if (!participant) {
            return errorResponse('Participant not found in that session.', 404)
          }
    ...
    websocket: {
      open(socket) {
        ...
        if (!record) {
          socket.send(
            JSON.stringify({ type: 'session.error', payload: { message: 'Join token is invalid.' } }),
          )
          socket.close()
          return
        }
        ...
        const participant = session?.participants.find(
          (candidate) => candidate.id === record.participantId,
        )
        ...
        if (!session || !participant) {
          socket.send(
            JSON.stringify({
              type: 'session.error',
              payload: { message: 'Shared session no longer exists.' },
            }),
          )
          socket.close()
          return
      ...
      message(socket, rawMessage) {
        ...
        try {
          ...
          socket.send(
            JSON.stringify({
              type: 'session.error',
              payload: { message: 'Shared session message was not valid JSON.' },
            }),
          )
          return
        }
        ...
        if (message.type === 'participant.status') {
          ...
          if (!participant.twitchIdentity && message.payload.twitchLogin) {
            participant.twitchIdentity = {
              ...
              displayName:
                participant.twitchIdentity?.displayName ?? message.payload.twitchLogin,
            }
          }
        }
        ...
        if (message.type === 'timer.action') {
          if (participant.id !== session.hostParticipantId) {
            socket.send(
              JSON.stringify({
                type: 'session.error',
                payload: { message: 'Only the host can change the shared timer right now.' },
              }),
            )
            return
          }

          session.timerState = applyTimerAction(session.timerState, message.payload, defaultTimerSeconds)
        }
        ...
        if (message.type === 'wheel.action') {
          ...
          if (
            ...
            socket.send(
              JSON.stringify({
                type: 'session.error',
                payload: { message: 'That wheel result is no longer owned by this desktop.' },
              }),
            )
            return
          }

          const selectedSegment = session.wheelSegments.find(
            (segment) => segment.id === currentSpin.activeSegmentId,
          )
          if (!selectedSegment || selectedSegment.outcomeType !== 'timeout') {
            socket.send(
              JSON.stringify({
                type: 'session.error',
                payload: {
                  message: 'That shared wheel result cannot be applied as a timeout.',
                },
              }),
            )
            return
        ...
        if (message.type === 'session.end') {
          if (participant.id !== session.hostParticipantId) {
            socket.send(
              JSON.stringify({
                type: 'session.error',
                payload: { message: 'Only the host can end the shared session.' },
              }),
            )
            return
          }
  ...
  async function stop() {
    for (const sessionId of wheelRevealTimers.keys()) {
      clearWheelRevealTimer(sessionId)
    }

    for (const sessionId of wheelAutoApplyTimers.keys()) {
      clearWheelAutoApplyTimer(sessionId)
    }
